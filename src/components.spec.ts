...
import { _setTestKey, component, html, mount, componentRegistry } from "./cr-26";
import { log } from "./log";
...
describe("cr-26 components", () => {
  ...
  it("should remove references when an existing component is not rendered", () => {
    const testRefs = (expectedIds: string[]) => {
      const refIds = Array.from(componentRegistry.keys()).sort();
  ...
  it("should keep instance.state as undefined for stateless components", () => {
    let statelessAction: Function = () => {};
    ...
    // Verify stateless component has undefined state initially
    const statelessInstance = componentRegistry.get("stateless");
    expect(statelessInstance?.state).toBeUndefined();

    // Execute action on stateless component
    statelessAction(testKey);

    // Verify state remains undefined after action execution
    expect(statelessInstance?.state).toBeUndefined();

    // Verify stateful component has state
    const appInstance = componentRegistry.get("app");
    expect(appInstance?.state).toBeDefined();
    expect(appInstance?.state).toEqual({ count: 0 });
  });
});
